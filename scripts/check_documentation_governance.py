...
DOC-012 binds the human documentation layer to the durable post-Freesound
corpus-closure evidence. Exact observed identities/counts are intentionally
frozen; any empirical change requires a fresh documentation audit.
"""
...
def main() -> int:
    ...
    json_names = {"sonyc_cert", "ledger_summary", "dedup", "group_audit", "split", "coverage", "readiness", "freeze1", "freeze2", "repro"}
    ...
    # DOC-012 lineage and human authorities.
    ...
    require("CERT-DOC-012 = CERTIFIED / current" in docs["state"], "CURRENT-STATE does not mark DOC-012 current", failures)
    require("**Certificate:** `CERT-DOC-012`" in docs["doc_audit"] and "**Status:** `CERTIFIED`" in docs["doc_audit"], "DOC-012 audit is not certified", failures)
    require(EXPECTED_READINESS_COMMIT in docs["doc_audit"], "DOC-012 does not bind current readiness commit", failures)
    require(EXPECTED_EVIDENCE_IDENTITY in docs["doc_audit"], "DOC-012 does not bind current readiness identity", failures)
    require(ledger_state(docs["cert_ledger"], "CERT-DOC-012", "CERTIFIED"), "CERT-DOC-012 not CERTIFIED in certification ledger", failures)
    ...
    # Current canonical ledger; live-source drift requires a new audit.
    ...
    # Structural closure and protected split conflicts.
    ...
    require(split.get("original_split_conflict_count") == 2, "protected split conflict count drift", failures)
    require(split.get("original_split_conflicts_quarantined") == 2, "protected split conflicts are not fully quarantined", failures)
    require(split.get("quarantined_asset_count") == 93, "split quarantine asset count drift", failures)

    # Exact final coverage snapshot.
    require(coverage.get("status") == "FAIL", "coverage is no longer FAIL; a fresh audit is required", failures)
    ...
    require(coverage.get("development_asset_count") == 1048, "development asset count drift", failures)
    ...
    require((glass_hn.get("asset_count"), glass_hn.get("independent_group_count"), glass_hn.get("source_count")) == (428, 408, 2), "GLASS hard-negative closure drift", failures)
    ...
    # Freeze/reproducibility remain blocked only by upstream corpus closure.
    ...
    require((targets.get("GLASS_SHATTER") or {}).get("positive_assets_pre_final_dedup") == 303, "readiness GLASS pre-final count drift", failures)

    # Human docs preserve the downstream stop line.
    ...
    print(f" certificate=CERT-DOC-012 markdown_files={len(markdown_files)}")
