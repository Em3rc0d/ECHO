...
def enrich_row(row: dict[str, Any], evidence: Mapping[str, Any] | None, source_policy: Mapping[str, Any]) -> None:
    blockers = {
...
def main() -> int:
    for path in (LEDGER, SUMMARY, SOURCE_POLICY_PATH, SONYC_TARGETS, SONYC_CONFUSERS, FREESOUND, PUBLIC_GAP):
        if not path.is_file():
    ...
    for row in rows:
        ...
        if evidence:
            matched += 1
        enrich_row(row, evidence, source_policy)
    ...
    summary["enrichment"] = {
        ...
        "inputs": {
            ...
            for path in (SONYC_TARGETS, SONYC_CONFUSERS, FREESOUND, PUBLIC_GAP)
        },
        ...
        "hard_negative_stop_line": "FSD50K confuser labels create hard-negative roles only; they never promote target positives."
    }
