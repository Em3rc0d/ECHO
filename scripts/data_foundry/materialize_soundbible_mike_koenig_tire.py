...
OUTPUT_ROOT = Path(os.environ.get("ECHO_SOUNDBIBLE_TIRE_ROOT", ROOT / ".materialized-soundbible-mike-koenig-tire"))
...
def fetch(url: str, attempts: int = 6) -> tuple[bytes, str | None, str]:
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            with urlopen(request, timeout=120) as response:  # nosec B310 - governed public evidence URL
...
def main() -> int:
    ...
    audio_files = sorted(path for path in extract_root.rglob("*") if path.is_file() and path.suffix.casefold() in allowed)
